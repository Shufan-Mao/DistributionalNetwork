...
from src.MissingAdjunct.items import LogicalForm
from src.MissingAdjunct.items import agent_classes, theme_classes, experimental_themes
...
class Corpus:
    ...
    def get_sentences(self,
        ...
        for lf in self.get_logical_forms():
            ...
            if lf.instrument:
                if self.add_with:
                    sentence = WITH + WS + lf.instrument + WS + sentence
                else:
                    sentence = lf.instrument + WS + sentence

            if lf.location:
                if self.add_in:
                    sentence = IN + WS + lf.location + WS + sentence
                else:
                    sentence = lf.location + WS + sentence
...
corpus = Corpus(include_location=False,
                ...
                add_with=True,
                add_in=False,
                strict_compositional=False,  # todo test
                )

for sentence in corpus.get_sentences():
    if 'pepper' in sentence or 'potato' in sentence:
        print(sentence)
'''''''''
